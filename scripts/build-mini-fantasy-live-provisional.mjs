...
function emptyLiveProvisionalFile() {
  return {
...
function createLiveTemplate(fixture = {}) {
  return {
...
async function main() {
  ...
  if (options.clear) {
    await writeJson(DATA_FILE, emptyLiveProvisionalFile());
  ...
  if (options.init) {
    ...
    await writeJson(outputPath, createLiveTemplate(fixture));
  ...
  const summary = buildLiveSummary(payload);
