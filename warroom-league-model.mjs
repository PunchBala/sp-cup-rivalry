export const LEAGUE_MODEL_VERSION = 1;
export const VALID_TEMPLATE_IDS = ['ipl-classic-v1'];
export const VALID_VISIBILITY = ['public', 'unlisted', 'private'];
export const VALID_STATES = ['draft', 'locked'];
...
function validatePlayerArray(players, errors, warnings) {
  if (!Array.isArray(players) || players.length < 2) {
    ...
    return { playerIds: new Set(), playerNames: new Set() };
  }
  ...
  players.forEach((player, index) => {
    ...
    if (id && name) {
      ...
      if (generatedId !== id) {
        warnings.push(`${path}: id '${id}' does not match generated id '${generatedId}' from name`);
      }
    }
  });
...
function validatePickObject(pickObject, path, errors, { requireCompletePicks }) {
  ...
  for (const categoryKey of VALID_CATEGORY_KEYS) {
    if (!isNonEmptyString(pickObject[categoryKey])) {
      pushError(errors, `${path}.${categoryKey}`, 'is required once the league is locked');
    }
  }
}

export function validateLeagueConfig(league, options = {}) {
  const errors = [];
  const warnings = [];
  ...
  if (Number(league.version) !== LEAGUE_MODEL_VERSION) {
    pushError(errors, 'version', `must be ${LEAGUE_MODEL_VERSION}`);
  }
  ...
  if (league.id && slug && normalizeSlug(league.id) !== slug) {
    warnings.push(`id '${league.id}' and slug '${league.slug}' normalize differently`);
  }

  if (!VALID_TEMPLATE_ID_SET.has(league.templateId)) {
    pushError(errors, 'templateId', `must be one of: ${VALID_TEMPLATE_IDS.join(', ')}`);
  }

  if (!VALID_VISIBILITY_SET.has(league.visibility)) {
    pushError(errors, 'visibility', `must be one of: ${VALID_VISIBILITY.join(', ')}`);
  }

  if (!VALID_STATE_SET.has(league.state)) {
    pushError(errors, 'state', `must be one of: ${VALID_STATES.join(', ')}`);
  }
  ...
  if (!league.picks || typeof league.picks !== 'object' || Array.isArray(league.picks)) {
    ...
    for (const playerId of playerIds) {
      validatePickObject(league.picks[playerId], `picks.${playerId}`, errors, { requireCompletePicks });
    }

    for (const pickOwnerId of Object.keys(league.picks)) {
      if (!playerIds.has(pickOwnerId)) {
        pushError(errors, `picks.${pickOwnerId}`, 'has no matching player in players[]');
      }
    }
  }

  if (league.settings != null && (typeof league.settings !== 'object' || Array.isArray(league.settings))) {
    pushError(errors, 'settings', 'must be an object when provided');
  }

  return { ok: errors.length === 0, errors, warnings };
}

export function assertValidLeagueConfig(league, options = {}) {
  const result = validateLeagueConfig(league, options);
  if (!result.ok) {
    const message = ['League validation failed:', ...result.errors.map((line) => `- ${line}`)].join('\n');
...
export function normalizeLeagueConfig(league) {
  ...
  cloned.players = cloned.players.map((player) => ({
    ...player,
    id: normalizeSlug(player.id),
    name: normalizeWhitespace(player.name)
  }));
  return cloned;
}

export function buildMatchupId(aId, bId) {
  return `${normalizeSlug(aId)}-${normalizeSlug(bId)}`;
}

export function buildMatchupsFromLeague(league) {
...
export function createLocalLeagueAdapter(fixturesBySlug = {}) {
  ...
  return {
    listLeagueSlugs() {
      return Object.keys(normalizedFixtures).sort();
    },
